...
class Model:
    def __init__(self, args, training=True):
        ...
        # inputs = tf.unstack(inputs, num=args.seq_length, axis=1)
        ...
        #     output, last_state = cell(input_, last_state)
        #     outputs.append(output)
        with tf.variable_scope("RNN"):
            for time_step in range(args.seq_length):
                if time_step > 0:
                    tf.get_variable_scope().reuse_variables()
                cell_input = inputs[:, time_step, :]
                cell_output, last_state = cell(cell_input, last_state)
    ...
    def sample(self, sess, chars, vocab, num, sampling_type=1):
        prime = "password\n"
        ...
        # for n in range(num):
        print("Predict Password number: %d" % num)
        n = 0
        out = open("out.txt", "w")
        while n < num:
            x = np.zeros((1, 1))
